...
class _ph {
 ...
 public:
    ...
    constexpr explicit _ph(const _detail::any_holder<const Input*>& input, _detail::any_holder<Sink>& sink)
    ...
    constexpr auto get_result(bool rule_parse_result, T&& result) && {
        ...
        return Result(LEXY_MOV(_validate).template get_result<validate_result>(rule_parse_result), LEXY_MOV(result));
    }
