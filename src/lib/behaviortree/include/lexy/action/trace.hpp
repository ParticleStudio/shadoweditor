 ...
 public:
    ...
    void write_error(const Location& loc, const lexy::error<Reader, Tag>& error) {
        ...
        if constexpr(std::is_same_v<Tag, lexy::expected_literal>) {
            auto string = _detail::make_literal_lexeme<typename Reader::encoding>(error.string(), error.length());
    ...
    class event_handler {
     ...
     public:
        ...
        void on(_th& handler, parse_events::token, TK kind, iterator begin, iterator end) {
            ...
            handler._writer.write_token(loc, token_kind<TokenKind>(kind), lexeme_for<Input>(begin, end));
        }
...
struct trace_action {
    ...
    constexpr auto operator()(Production, const Input& input) const {
        ...
        return lexy::do_action<Production, result_type>(handler(_out, input, _opts), _state, reader);
    }
};
...
OutputIt trace_to(OutputIt out, const Input& input, const State& state, visualization_options opts = {}) {
    return trace_action<const State, Input, OutputIt, TokenKind>(state, out, opts)(Production {}, input);
}
