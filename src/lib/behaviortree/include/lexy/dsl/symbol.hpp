...
class _symbol_table {
 ...
 public:
    ...
    LEXY_CONSTEVAL auto case_folding(CaseFoldingDSL) const {
        return _symbol_table<T, CaseFoldingDSL::template case_folding, Strings...>(_detail::make_index_sequence<size()> {}, *this);
    }
...
#else
#if(defined(__clang__) && __clang_major__ <= 7) || (defined(__clang__) && defined(__apple_build_version__) && __clang_major__ <= 10)
 ...
 private:
    ...
    constexpr explicit _symbol_table(lexy::_detail::index_sequence<Idx...>, const T* data, Args&&... args)
    ...
    constexpr explicit _symbol_table(lexy::_detail::index_sequence<Idx...>, const _symbol_table<T, OtherCaseFolding, Strings...>& table)
...
struct _sym: branch_base {
    ...
    struct p {
        ...
        struct _cont {
            ...
            LEXY_PARSER_FUNC static bool parse(Context& context, Reader& reader, PrevArgs&&... args, lexy::lexeme<Reader> lexeme) {
        ...
        LEXY_PARSER_FUNC static bool parse(Context& context, Reader& reader, Args&&... args) {
            ...
            return lexy::parser_for<_cap<Token>, _cont<Args...>>::parse(context, reader, LEXY_FWD(args)...);
        }
    };
...
struct _sym_dsl: _sym<Table, void, void> {
    ...
    constexpr auto operator()(_id<L, T, R...> id) const {
        static_assert(sizeof...(R) == 0, "symbol() must not be used in the presence of reserved identifiers");
