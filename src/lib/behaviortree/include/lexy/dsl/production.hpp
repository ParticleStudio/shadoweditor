...
/* not force inline */ constexpr bool _finish_production(ProductionParser& parser, Context& context, Reader& reader) {
...
struct _prd
    ...
    : std::conditional_t<lexy::_production_defines_whitespace<Production>, rule_base, _copy_base<lexy::production_rule<Production>>> {
    ...
    struct p {
        ...
        LEXY_PARSER_FUNC static bool parse(Context& context, Reader& reader, Args&&... args) {
            ...
            if constexpr(lexy::_production_defines_whitespace<Production>) {
                if(!lexy::whitespace_parser<decltype(sub_context), lexy::pattern_parser<>>::parse(sub_context, reader)) {
...
struct _recb: branch_base {
    ...
    struct _depth_handler {
        ...
        static constexpr bool increment_depth(Context& context, Reader& reader) {
            ...
            LEXY_ASSERT(control_block->max_depth > 0, "dsl::recurse_branch<P> is disabled in this context");
    ...
    struct p {
        ...
        LEXY_PARSER_FUNC static bool parse(Context& context, Reader& reader, Args&&... args) {
            ...
            return lexy::parser_for<_prd<Production>, depth>::parse(context, reader, LEXY_FWD(args)...);
        }
    };
