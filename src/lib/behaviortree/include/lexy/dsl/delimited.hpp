...
struct _del_chars {
    ...
    constexpr void parse_swar(Reader& reader, Close, Escs...) {
        ...
        if constexpr(lexy::_detail::is_swar_reader<Reader>//
                     && (lexy::is_literal_rule<Close> && ... && Escs::esc_is_literal) && !std::is_same_v<decltype(CharClass::template char_class_match_swar<encoding>({})), std::false_type>) {
            ...
            while(true) {
                ...
                if(swar_has_char<char_type, encoding::eof()>(cur) || swar_has_char<char_type, Close::template lit_first_char<encoding>()>(cur))
                    break;
    ...
    constexpr void parse_one(Context& context, Reader& reader, Sink& sink) {
        ...
        if(matcher::template match<encoding>(reader.peek())) {
            reader.bump();
        } else if constexpr(!std::is_same_v<decltype(CharClass::char_class_match_cp(char32_t())), std::false_type>) {
            ...
            if constexpr(std::is_same_v<encoding, lexy::default_encoding>//
                ...
                static_assert(!CharClass::char_class_unicode(), "cannot use this character class with default/byte_encoding");
                LEXY_ASSERT(reader.peek() != encoding::eof(), "EOF should be checked before calling this");
            ...
            // Otherwise, try to match Unicode characters.
                ...
                static_assert(CharClass::char_class_unicode(), "cannot use this character class with Unicode encoding");

                auto result = lexy::_detail::parse_code_point(reader);
                if(result.error == lexy::_detail::cp_error::success && CharClass::char_class_match_cp(result.cp)) {
...
struct _del: rule_base {
    ...
    LEXY_PARSER_FUNC static bool _loop(CloseParser& close, Context& context, Reader& reader, Sink& sink) {
    ...
    struct p {
        ...
        LEXY_PARSER_FUNC static bool parse(Context& context, Reader& reader, Args&&... args) {
            ...
            if constexpr(std::is_same_v<typename decltype(sink)::return_type, void>) {
                ...
                return close.template finish<NextParser>(context, reader, LEXY_FWD(args)..., LEXY_MOV(sink).finish());
            }
        }
    };
};
