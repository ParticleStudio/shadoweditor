...
struct _comb_it: rule_base {
    ...
    struct p {
        ...
        LEXY_PARSER_FUNC static bool parse(Context&, Reader&, _comb_control<Sink>& ctrl, Args&&... args) {
            ctrl.idx = Idx;
    ...
    struct p {
        ...
        LEXY_PARSER_FUNC static bool parse(Context& context, Reader& reader, Args&&... args) {
            ...
            for(auto count = 0; count < int(N); ++count) {
                ...
                using parser = lexy::parser_for<_comb_choice, lexy::pattern_parser<decltype(control)>>;
...
constexpr auto combination(R...) {
    ...
    static_assert((!lexy::is_unconditional_branch_rule<R> && ...), "combination() does not support unconditional branches");
...
constexpr auto partial_combination(R...) {
    ...
    static_assert((!lexy::is_unconditional_branch_rule<R> && ...), "partial_combination() does not support unconditional branches");
