...
struct _mem: _copy_base<Rule> {
    ...
    struct bp {
        ...
        LEXY_PARSER_FUNC auto finish(Context& context, Reader& reader, Args&&... args) {
            // Add member tag here.
            return rule.template finish<NextParser>(context, reader, LEXY_FWD(args)..., lexy::member<Fn> {});
        }
    };
...
struct _mem_dsl {
    ...
    {
        using lambda = std::conditional_t<std::is_default_constructible_v<Fn>, Fn, lexy::_detail::stateless_lambda<Fn>>;
