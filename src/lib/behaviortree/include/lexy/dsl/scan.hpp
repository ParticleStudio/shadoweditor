...
class scanner {
 public:
    ...
    constexpr void parse(scan_result<T>& result, Rule) {
        ...
        auto success = parser::parse(static_cast<Derived&>(*this).context(), _reader, &result._value);
        if(!success)
    ...
    constexpr auto parse(Production = {}) {
        ...
        using value_type =
                typename lexy::production_value_callback<Production, typename context_t::state_type>::return_type;
    ...
    constexpr bool branch(scan_result<T>& result, Rule) {
        ...
        auto success = parser.template finish<lexy::_detail::scan_final_parser>(context, _reader, &result._value);
        if(!success)
            _state = _state_failed;
...
class rule_scanner: public _detail::scanner<rule_scanner<Context, Reader>, Reader> {
 ...
 private:
    constexpr explicit rule_scanner(Context& context, Reader reader)
        : _detail::scanner<rule_scanner<Context, Reader>, Reader>(reader), _context(&context), _begin(reader.position()) {}
...
struct _scan: rule_base {
    ...
    struct p {
        ...
        LEXY_PARSER_FUNC static bool _parse(Scanner& scanner, Context& context, Reader& reader, Args&&... args) {
            lexy::scan_result result = [&] {
                if constexpr(lexy::_detail::is_detected<
                    ...
                    return Context::production::scan(scanner, *context.control_block->parse_state, LEXY_FWD(args)...);
                else
