...
struct _digits_st: token_base<_digits_st<Base, Sep>> {
    ...
    struct tp {
        ...
        constexpr bool try_parse(Reader reader) {
            ...
            if(result && lexy::_detail::next(begin) != end && *begin == lexy::_detail::transcode_char<char_type>('0')) {
        ...
        constexpr void report_error(Context& context, const Reader& reader) {
            if(forbidden_leading_zero) {
                auto err = lexy::error<Reader, lexy::forbidden_leading_zero>(reader.position(), end);
...
struct _digits_t: token_base<_digits_t<Base>> {
    ...
    struct tp {
        ...
        constexpr void report_error(Context& context, const Reader& reader) {
            if(forbidden_leading_zero) {
                auto err = lexy::error<Reader, lexy::forbidden_leading_zero>(reader.position(), this->end);
                context.on(_ev::error {}, err);
            } else {
                auto err = lexy::error<Reader, lexy::expected_char_class>(reader.position(), Base::char_class_name());
                context.on(_ev::error {}, err);
            }
        }
    };
...
struct _ndigits_s: token_base<_ndigits_s<N, Base, Sep>> {
    ...
    struct tp<Reader, lexy::_detail::index_sequence<Idx...>> {
        ...
        constexpr bool try_parse(Reader reader) {
            ...
            auto success = (((void)Idx, lexy::try_match_token(Sep {}, reader), lexy::try_match_token(digit<Base>, reader)) && ...);
            end = reader.position();
            return success;
        }
