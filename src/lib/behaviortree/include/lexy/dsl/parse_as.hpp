...
struct _pas: _copy_base<Rule> {
    ...
    struct bp {
        ...
        LEXY_PARSER_FUNC bool finish(Context& context, Reader& reader, Args&&... args) {
            ...
            using context_type = lexy::_pc<decltype(handler), typename Context::state_type, typename Context::production, typename Context::whitespace_production>;
            context_type sub_context(&cb);

            auto result = rule_parser.template finish<lexy::_detail::final_parser>(sub_context, reader);
            ...
            if(!result)
                ...
                return NextParser::parse(context, reader, *LEXY_MOV(sub_context.value), LEXY_FWD(args)...);
            else
                return NextParser::parse(context, reader, LEXY_FWD(args)..., *LEXY_MOV(sub_context.value));
        }
    };
    ...
    struct p {
        ...
        LEXY_PARSER_FUNC static bool parse(Context& context, Reader& reader, Args&&... args) {
            ...
            auto result = lexy::parser_for<Rule, lexy::_detail::final_parser>::parse(sub_context, reader);
