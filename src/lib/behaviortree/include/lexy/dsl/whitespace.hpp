...
constexpr auto skip_whitespace(ws_handler<Handler>&& handler, Reader& reader) {
    ...
    if constexpr(lexy::is_token_rule<WhitespaceRule>) {
        ...
        handler.real_on(lexy::parse_events::token {}, lexy::whitespace_token_kind, begin, reader.position());
        ...
        auto result = lexy::do_action<production, ws_result>(LEXY_MOV(handler), lexy::no_parse_state, reader);

        handler.real_on(lexy::parse_events::token {}, result ? lexy::whitespace_token_kind : lexy::error_token_kind, begin, reader.position());
        return result;
    } else {
        (void)handler;
        (void)reader;
    ...
    LEXY_PARSER_FUNC static bool parse(Context& context, Reader& reader, Args&&... args) {
        if(!std::is_base_of_v<disable_whitespace_skipping, NextParser>//
           && context.control_block->enable_whitespace_skipping) {
            using whitespace = lexy::production_whitespace<typename Context::production, typename Context::whitespace_production>;
...
struct _wsr: rule_base {
    ...
    struct p {
        ...
        constexpr static bool parse(Context& context, Reader& reader, Args&&... args) {
            auto result = lexy::_detail::skip_whitespace<Rule>(lexy::_detail::ws_handler(context), reader);
            if(!result)
                return false;
...
struct _wsn: _copy_base<Rule> {
    ...
    struct _pc {
        ...
        LEXY_PARSER_FUNC static bool parse(Context& context, Reader& reader, Args&&... args) {
            ...
            return lexy::whitespace_parser<Context, NextParser>::parse(context, reader, LEXY_FWD(args)...);
        }
    };
