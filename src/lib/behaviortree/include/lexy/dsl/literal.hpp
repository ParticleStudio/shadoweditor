...
struct lit_trie {
    ...
    static constexpr auto max_transition_count = max_node_count == 1 ? 1 : max_node_count - 1;// it is a tree
    ...
    LEXY_CONSTEVAL lit_trie()
        : node_count(1), node_value {}, node_char_class {}, transition_char {}, transition_from {}, transition_to {} {
...
struct _merge_case_folding<CurrentCaseFolding, H, T...>
    : _merge_case_folding<std::conditional_t<std::is_void_v<CurrentCaseFolding>, typename H::lit_case_folding, CurrentCaseFolding>, T...> {
    static_assert(
...
LEXY_CONSTEVAL auto make_empty_trie() {
    ...
    constexpr auto char_classes = (lexy::_detail::char_class_list {} + ... + Literals::lit_char_classes);
...
struct lit_trie_matcher<Trie, CurNode> {
    ...
    LEXY_FORCE_INLINE static constexpr bool _try_transition(std::size_t& result, Reader& reader, IntT cur) {
...
template<typename CharT, CharT... C>
struct _lit
    : token_base<_lit<CharT, C...>, std::conditional_t<sizeof...(C) == 0, unconditional_branch_base, branch_base>>,
      _lit_base {
...
struct _lcp: token_base<_lcp<Cp...>>, _lit_base {
    ...
    template<typename Reader, typename Indices = lexy::_detail::make_index_sequence<_string<typename Reader::encoding>.length>>
    struct tp;
    ...
    struct tp<Reader, lexy::_detail::index_sequence<Idx...>> {
        ...
        constexpr bool try_parse(Reader reader) {
            ...
            auto result = lexy::_detail::match_literal<0, typename encoding::char_type, _string<encoding>.data[Idx]...>(reader);
            end = reader.position();
            return result;
        }
        ...
        constexpr void report_error(Context& context, const Reader& reader) {
            ...
            auto err = lexy::error<Reader, lexy::expected_literal>(begin, _string<encoding>.data, index, _string<encoding>.length);
            context.on(_ev::error {}, err);
        }
    };
};
