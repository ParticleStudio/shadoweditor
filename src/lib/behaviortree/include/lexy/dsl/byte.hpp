...
struct _b: token_base<_b<N>> {
    ...
    struct tp<Reader, lexy::_detail::index_sequence<Idx...>> {
        ...
        constexpr bool try_parse(Reader reader) {
            ...
            // Bump N times.
            auto result = ((reader.peek() == Reader::encoding::eof() ? ((void)Idx, false) : (reader.bump(), true)) && ...);
            end = reader.position();
            return result;
        }
...
struct _bint: branch_base {
    ...
    struct _pc<NextParser, lexy::_detail::index_sequence<Idx...>> {
        ...
        LEXY_PARSER_FUNC static bool parse(Context& context, Reader& reader, typename Reader::iterator begin, typename Reader::iterator end, Args&&... args) {
            ...
            return lexy::whitespace_parser<Context, NextParser>::parse(context, reader, LEXY_FWD(args)..., result);
        }
    };
