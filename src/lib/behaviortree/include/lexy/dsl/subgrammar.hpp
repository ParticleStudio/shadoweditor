...
#define LEXY_DECLARE_SUBGRAMMAR(Production)                                                                                                   \
    ...
    struct _subgrammar<Production, Handler, State, Reader> {                                                                                  \
        template<typename T>                                                                                                                  \
        static bool parse(_detail::lazy_init<T>& value, _detail::parse_context_control_block<Handler, State>* control_block, Reader& reader); \
    };                                                                                                                                        \
    }
...
#define LEXY_INSTANTIATE_SUBGRAMMAR(Production, ...)                                                                             \
    template bool ::lexy::_subgrammar_for<Production, __VA_ARGS__>                                                          /**/ \
            ::parse<::lexy::_production_value_type<typename __VA_ARGS__::handler, typename __VA_ARGS__::state, Production>> /**/ \
            (::lexy::_detail::lazy_init<::lexy::_production_value_type<                                                          \
                     typename __VA_ARGS__::handler, typename __VA_ARGS__::state, Production>>&,                                  \
             ::lexy::_detail::parse_context_control_block<typename __VA_ARGS__::handler, typename __VA_ARGS__::state>*,          \
...
struct _subg: rule_base {
    ...
    struct p {
        ...
        LEXY_PARSER_FUNC static bool parse(Context& context, Reader& reader, Args&&... args) {
            ...
            using subgrammar_traits = lexy::_subgrammar<Production, handler_type, state_type, Reader>;
            auto rule_result = subgrammar_traits::template parse<value_type>(value, context.control_block, reader);
