...
struct integer_traits {
    ...
    static constexpr auto _max = [] {
        if constexpr(std::is_same_v<T, char>)
            ...
            static_assert(_detail::error<T>, "specialize integer_traits for your custom integer types");
    }();
    template<int Radix>
...
struct _int: _copy_base<Token> {
    ...
    struct _pc {
        ...
        LEXY_PARSER_FUNC static bool parse(Context& context, Reader& reader, typename Reader::iterator begin, typename Reader::iterator end, Args&&... args) {
            ...
            return lexy::whitespace_parser<Context, NextParser>::parse(context, reader, LEXY_FWD(args)..., value);
        }
    };
...
constexpr auto code_point_id = [] {
    using type = std::conditional_t<_ndigits_can_overflow<lexy::code_point, N, Base::digit_radix>(), lexy::code_point, lexy::unbounded<lexy::code_point>>;
...
constexpr auto code_unit_id = [] {
    ...
    using type = std::conditional_t<_ndigits_can_overflow<char_type, N, Base::digit_radix>(), char_type, lexy::unbounded<char_type>>;
    using parser = _integer_parser<type, Base, true>;
