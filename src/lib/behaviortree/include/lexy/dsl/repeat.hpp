...
struct _rep_impl {
    ...
    static constexpr bool loop(Context& context, Reader& reader, std::size_t count, Sink&... sink) {
        using sink_parser = std::conditional_t<sizeof...(Sink) == 0, lexy::pattern_parser<>, lexy::sink_parser>;
...
struct _repd: rule_base// repeat, discard
{
    ...
    struct p {
        ...
        LEXY_PARSER_FUNC static bool parse(Context& context, Reader& reader, std::size_t count, Args&&... args) {
...
struct _repc: rule_base// repeat, capture
{
    ...
    struct p {
        ...
        LEXY_PARSER_FUNC static bool parse(Context& context, Reader& reader, std::size_t count, Args&&... args) {
            ...
            return NextParser::parse(context, reader, LEXY_FWD(args)..., lexy::lexeme(reader, begin));
        }
    };
};
