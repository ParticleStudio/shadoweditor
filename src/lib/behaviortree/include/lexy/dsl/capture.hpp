...
struct _cap: _copy_base<Token> {
    ...
    struct bp {
        ...
        LEXY_PARSER_FUNC auto finish(Context& context, Reader& reader, Args&&... args) {
            ...
            return continuation::parse(context, reader, LEXY_FWD(args)..., lexy::lexeme<Reader>(begin, end));
        }
    };
...
struct _capr: _copy_base<Rule> {
    ...
    struct _pc: lexy::_detail::disable_whitespace_skipping {
        template<typename Context, typename Reader, typename... Args>
        LEXY_PARSER_FUNC static bool parse(Context& context, Reader& reader, PrevArgs&&... prev_args, typename Reader::iterator begin, Args&&... args) {
            using continuation = lexy::whitespace_parser<Context, NextParser>;
            return continuation::parse(context, reader, LEXY_FWD(prev_args)..., lexy::lexeme(reader, begin), LEXY_FWD(args)...);
        }
    };
    ...
    struct bp {
        ...
        LEXY_PARSER_FUNC auto finish(Context& context, Reader& reader, Args&&... args) {
            ...
            return rule.template finish<continuation>(context, reader, LEXY_FWD(args)..., reader.position());
        }
    };
