...
struct _recovery_wrapper: _recovery_base {
    ...
    struct p {
        struct _continuation {
            ...
            LEXY_PARSER_FUNC static bool parse(Context& context, Reader& reader, bool& recovery_finished, Args&&... args) {
        ...
        LEXY_PARSER_FUNC static bool parse(Context& context, Reader& reader, Args&&... args) {
            ...
            auto result = lexy::parser_for<Rule, _continuation>::parse(context, reader, recovery_finished, LEXY_FWD(args)...);
...
struct _find: _recovery_base {
    ...
    struct p {
        ...
        LEXY_PARSER_FUNC static bool parse(Context& context, Reader& reader, Args&&... args) {
            ...
            constexpr const auto& trie = _look_trie<typename Reader::encoding, Token, decltype(get_limit())>;
...
struct _tryt: rule_base {
    ...
    struct _pc {
        template<typename Context, typename Reader, typename... Args>
        LEXY_PARSER_FUNC static bool parse(Context& context, Reader& reader, bool& continuation_reached, Args&&... args) {
            ...
            if constexpr(std::is_void_v<Terminator>)
                ...
                return lexy::parser_for<Terminator, NextParser>::parse(context, reader, LEXY_FWD(args)...);
        }
        ...
        LEXY_PARSER_FUNC static bool recover(Context& context, Reader& reader, Args&&... args) {
            if constexpr(std::is_void_v<Recover>)
                ...
                return lexy::parser_for<Recover, NextParser>::parse(context, reader, LEXY_FWD(args)...);
            else
                return lexy::parser_for<_recovery_wrapper<Recover>, NextParser>::parse(context, reader, LEXY_FWD(args)...);
        }
    };
...
struct _tryr: _copy_base<Rule> {
    ...
    struct bp {
        ...
        LEXY_PARSER_FUNC bool finish(Context& context, Reader& reader, Args&&... args) {
            ...
            auto continuation_reached = false;
            auto result = rule.template finish<continuation>(context, reader, continuation_reached, LEXY_FWD(args)...);
            if(continuation_reached)
                // Whatever happened, it is not our problem as we've reached the continuation.
                return result;
