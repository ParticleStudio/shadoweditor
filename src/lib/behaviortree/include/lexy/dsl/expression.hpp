...
struct operation_list {
    ...
    static constexpr bool apply(Context& context, Reader& reader, parsed_operator<Reader> op, Args&&... args) {
        ...
        (void)((cur_idx <= op.idx && op.idx < cur_idx + op_of<Operations>::op_literals::size
                        ? (result = Continuation<Operations>::parse(context, reader, parsed_operator<Reader> {op.pos, op.idx - cur_idx}, LEXY_FWD(args)...),
                           true)
...
struct _operation_list_of {
    ...
    static constexpr auto get(Operation) {
        ...
        if constexpr(is_prefix == Pre && ((is_prefix && bp.rhs >= MinBindingPower) || (!is_prefix && bp.lhs >= MinBindingPower)))
...
struct _expr: rule_base {
    ...
    struct _continuation {
        struct _op_cont {
            ...
            LEXY_PARSER_FUNC static bool parse(Context& context, Reader& reader, _state& state, Args&&... op_args) {
                using namespace lexy::_detail;

                constexpr auto value_type_void = std::is_void_v<typename Context::value_type>;
                constexpr auto binding_power = binding_power_of<typename Context::production>(Operation {});
                ...
                if constexpr(std::is_base_of_v<infix_op_list, Operation>) {
                    ...
                    while(true) {
                        ...
                        if(!op_rule::template op_finish<lexy::sink_parser>(context, reader, op, sink)) {
                            result = false;
                            break;
                        }
                    }
                    ...
                    if constexpr(value_type_void)
                        ...
                        context.value.emplace_result(context.value_callback(), LEXY_FWD(op_args)..., *LEXY_MOV(value));
                    ...
                    if constexpr(value_type_void)
                        ...
                        context.value.emplace_result(context.value_callback(), *LEXY_MOV(lhs), LEXY_FWD(op_args)..., *LEXY_MOV(rhs));
                    ...
                    if constexpr(value_type_void)
                        ...
                        context.value.emplace_result(context.value_callback(), *LEXY_MOV(value), LEXY_FWD(op_args)...);
                }
    ...
    static constexpr bool _parse_lhs(Context& context, Reader& reader, _state& state) {
        ...
        using atom_parser = lexy::parser_for<LEXY_DECAY_DECLTYPE(Context::production::atom), final_parser>;
    ...
    struct p {
        ...
        LEXY_PARSER_FUNC static bool parse(Context& context, Reader& reader) {
            ...
            // that.
            constexpr auto min_binding_power = binding_power.is_prefix() ? binding_power.rhs : binding_power.lhs;
