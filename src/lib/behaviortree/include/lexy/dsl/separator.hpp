...
struct _nsep: rule_base {
    ...
    struct p {
        struct _pc {
            ...
            LEXY_PARSER_FUNC static bool parse(Context& context, Reader& reader, typename Reader::iterator sep_begin, Args&&... args) {
        ...
        LEXY_PARSER_FUNC static bool parse(Context& context, Reader& reader, Args&&... args) {
            if(lexy::branch_parser_for<Branch, Reader> parser {};
               !parser.try_parse(context.control_block, reader)) {
                ...
                return parser.template finish<_pc>(context, reader, reader.position(), LEXY_FWD(args)...);
            }
        }
    };
};
