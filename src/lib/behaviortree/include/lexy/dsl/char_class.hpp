...
namespace lexy::_detail {
template<const auto& CompressedAsciiSet, typename = make_index_sequence<CompressedAsciiSet.range_count()>, typename = make_index_sequence<CompressedAsciiSet.single_count()>>
...
struct ascii_set_matcher<CompressedAsciiSet, index_sequence<RangeIdx...>, index_sequence<SingleIdx...>> {
    ...
    LEXY_FORCE_INLINE static constexpr bool match([[maybe_unused]] typename Encoding::int_type cur) {
        return
                ...
                ((to_int_type<Encoding>(CompressedAsciiSet.range_lower[RangeIdx]) <= cur && cur <= to_int_type<Encoding>(CompressedAsciiSet.range_upper[RangeIdx])) || ...)
...
struct char_class_base: token_base<Derived>, _char_class_base {
    ...
    struct tp {
        ...
        constexpr bool try_parse(Reader reader) {
            ...
            if constexpr(std::is_same_v<decltype(Derived::char_class_match_cp(char32_t())), std::false_type>) {
                return false;
                ...
                static_assert(!Derived::char_class_unicode(), "cannot use this character class with default/byte_encoding");
                ...
                static_assert(Derived::char_class_unicode(), "cannot use this character class with Unicode encoding");
...
template<typename CharT, CharT C, typename = std::enable_if_t<C <= 0x7F || std::is_same_v<CharT, char32_t> || std::is_same_v<CharT, unsigned char>>>
...
struct _calt: char_class_base<_calt<Cs...>> {
    ...
    static constexpr auto char_class_unicode() {
        ...
        static_assert(!non_unicode
                              ...
                              // only match ASCII.
                              || ((!Cs::char_class_unicode() || std::is_same_v<decltype(Cs::char_class_match_cp(0)), std::false_type>) && ...),
...
struct _cminus: char_class_base<_cminus<Set, Minus>> {
    ...
    static constexpr auto char_class_match_cp(char32_t cp) {
        if constexpr(std::is_same_v<decltype(Set::char_class_match_cp(cp)), std::false_type>)
            return std::false_type {};
        else if constexpr(std::is_same_v<decltype(Minus::char_class_match_cp(cp)), std::false_type>)
