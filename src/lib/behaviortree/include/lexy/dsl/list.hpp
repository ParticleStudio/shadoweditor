...
struct _lst: _copy_base<Item> {
    ...
    struct p {
        ...
        LEXY_PARSER_FUNC static bool parse(Context& context, Reader& reader, Args&&... args) {
            ...
            return lexy::sink_finish_parser<NextParser>::parse(context, reader, sink, LEXY_FWD(args)...);
        }
    };
...
constexpr auto list(Item) {
    static_assert(lexy::is_branch_rule<Item>, "list() without a separator requires a branch condition");
...
constexpr auto list(Item, _tsep<Sep>) {
    static_assert(lexy::is_branch_rule<Item>, "list() without a trailing separator requires a branch condition");
    return _lst<Item, _tsep<Sep>> {};
}

template<typename Item, typename Sep>
constexpr auto list(Item, _isep<Sep>) {
    static_assert(lexy::_detail::error<Item, Sep>, "list() does not support `dsl::ignore_trailing_sep()`");
    return _lst<Item, void> {};
}
}// namespace lexyd
...
struct _lstt: rule_base {
    ...
    LEXY_PARSER_FUNC static bool _loop(_state initial_state, TermParser& term, Context& context, Reader& reader, Sink& sink) {
            ...
            switch(state) {
                ...
                case _state::separator:
                    if constexpr(!std::is_void_v<Sep>) {
                        ...
                        if(lexy::parser_for<typename Sep::rule, lexy::sink_parser>::parse(context, reader, sink)) {
                            ...
                            if constexpr(lexy::is_branch_rule<Item>) {
                                lexy::branch_parser_for<Item, Reader> item {};
                                if(item.try_parse(context.control_block, reader) && item.template finish<lexy::sink_parser>(context, reader, sink)) {
    ...
    struct p {
        ...
        LEXY_PARSER_FUNC static bool parse(Context& context, Reader& reader, Args&&... args) {
            ...
            if constexpr(std::is_same_v<typename decltype(sink)::return_type, void>) {
                ...
                return term.template finish<NextParser>(context, reader, LEXY_FWD(args)..., LEXY_MOV(sink).finish());
            }
        }
    };
};
