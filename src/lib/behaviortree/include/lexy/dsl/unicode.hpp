...
struct _alnum: char_class_base<_alnum> {
    ...
    static LEXY_UNICODE_CONSTEXPR bool char_class_match_cp(char32_t cp) {
        return lexy::_detail::code_point_has_properties<LEXY_UNICODE_PROPERTY(alphabetic)>(cp) || lexy::code_point(cp).general_category() == lexy::code_point::decimal_number;
    }
...
struct _word: char_class_base<_word> {
    ...
    static LEXY_UNICODE_CONSTEXPR bool char_class_match_cp(char32_t cp) {
        ...
        if(cat == lexy::code_point::mark || cat == lexy::code_point::connector_punctuation || cat == lexy::code_point::decimal_number)
            return true;

        return lexy::_detail::code_point_has_properties<LEXY_UNICODE_PROPERTY(alphabetic), LEXY_UNICODE_PROPERTY(join_control)>(cp);
    }
...
struct _graph: char_class_base<_graph> {
    ...
    static LEXY_UNICODE_CONSTEXPR bool char_class_match_cp(char32_t _cp) {
        ...
        return !cp.is_control() && !cp.is_surrogate() && cp.general_category() != lexy::code_point::unassigned && !_space::char_class_match_cp(cp.value());
    }
...
struct _print: char_class_base<_print> {
    ...
    static LEXY_UNICODE_CONSTEXPR bool char_class_match_cp(char32_t cp) {
        ...
        return !_control::char_class_match_cp(cp) && (_blank::char_class_match_cp(cp) || _graph::char_class_match_cp(cp));
    }
