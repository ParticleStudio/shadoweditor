...
struct _op: branch_base {
    ...
    LEXY_PARSER_FUNC static bool op_finish(Context& context, Reader& reader, lexy::_detail::parsed_operator<Reader> op, Args&&... args) {
        context.on(_ev::token {}, typename Literal::token_type {}, op.pos, reader.position());

        using continuation = lexy::whitespace_parser<Context, lexy::parser_for<_seq_impl<R...>, NextParser>>;
        if constexpr(std::is_void_v<TagType>)
            ...
            return continuation::parse(context, reader, LEXY_FWD(args)..., op_tag_type(reader.position()));
        else
    ...
    struct bp {
        ...
        LEXY_PARSER_FUNC bool finish(Context& context, Reader& reader, Args&&... args) {
            ...
            if constexpr(std::is_void_v<TagType>)
                ...
                return impl.template finish<continuation>(context, reader, LEXY_FWD(args)..., op_tag_type(reader.position()));
            else
                return impl.template finish<continuation>(context, reader, LEXY_FWD(args)..., op_tag_type {});
        }
    };
    ...
    struct p {
        ...
        LEXY_PARSER_FUNC static bool parse(Context& context, Reader& reader, Args&&... args) {
            ...
            using continuation = lexy::parser_for<Literal, lexy::parser_for<_seq_impl<R...>, NextParser>>;
            if constexpr(std::is_void_v<TagType>)
                return continuation::parse(context, reader, LEXY_FWD(args)...);
            else if constexpr(lexy::_detail::is_detected<_detect_op_tag_ctor, op_tag_type, Reader>)
...
constexpr auto op(_br<Literal, R...>) {
    static_assert(lexy::is_literal_rule<Literal>, "condition in the operator must be a literal rule");
...
struct _opc: branch_base {
    ...
    LEXY_PARSER_FUNC static bool op_finish(Context& context, Reader& reader, lexy::_detail::parsed_operator<Reader> op, Args&&... args) {
        ...
        (void)((cur_idx == op.idx
                        ? (result = Ops::template op_finish<NextParser>(context, reader, op, LEXY_FWD(args)...),
                           true)
                        : (++cur_idx, false)) ||
               ...);
