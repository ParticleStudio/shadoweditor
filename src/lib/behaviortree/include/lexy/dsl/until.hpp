...
constexpr void _until_swar([[maybe_unused]] Reader& reader) {
    if constexpr(std::is_same_v<Condition, _nl>//
        ...
        while(true) {
            ...
            if(lexy::_detail::swar_has_char<char_type, Reader::encoding::eof()>(cur) || lexy::_detail::swar_has_char_less<char_type, 0xF>(cur))
                break;
            reader.bump_swar();
        }
    }
    ...
    struct tp {
        ...
        constexpr std::true_type try_parse(Reader reader) {
            while(true) {
                ...
                if(reader.peek() == Reader::encoding::eof() || lexy::try_match_token(Condition {}, reader)) {
...
struct _until: token_base<_until<Condition>> {
    ...
    struct tp {
        ...
        constexpr void report_error(Context& context, Reader reader) {
            ...
            LEXY_ASSERT(reader.peek() == Reader::encoding::eof(), "forgot to set end in try_parse()");
