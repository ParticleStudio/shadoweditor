...
#ifndef SHADOW_THREAD_POOL_HPP
#define SHADOW_THREAD_POOL_HPP
 ...
 */
using light_thread_pool = Pool<tp::none>;
 ...
 */
using priority_thread_pool = Pool<tp::priority>;
 ...
 */
using pause_thread_pool = Pool<tp::pause>;
 ...
 */
using wdc_thread_pool = Pool<tp::wait_deadlock_checks>;
...
public:
     ...
     */
    explicit Pool(const std::size_t num_threads) : Pool(num_threads, [] {}) {}
    ...
    explicit Pool(F&& init) : Pool(0, std::forward<F>(init))
    {
    }
     ...
     */
    template <BS_THREAD_POOL_INIT_FUNC_CONCEPT(F)>
    Pool(const std::size_t num_threads, F&& init)
    {
