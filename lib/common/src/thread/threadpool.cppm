...
class [[nodiscard]] ThreadInfoIndex {
    friend class shadow::thread::ThreadPool;
...
public:
     ...
     */
    [[nodiscard]] OptionalIndex operator()() const { return m_index; }
...
public:
    ...
    [[nodiscard]] std::conditional_t<std::is_void_v<T>, void, std::vector<T>> Get() {
        if constexpr(std::is_void_v<T>) {
            for(std::future<T> &rFuture: *this) { rFuture.get(); }
            return;
        } else {
            std::vector<T> resultVec;
            resultVec.reserve(this->size());
            for(std::future<T> &rFuture: *this) { resultVec.push_back(rFuture.get()); }
            return resultVec;
        }
    }
    ...
    [[nodiscard]] std::size_t ReadyNum() const {
        std::size_t num = 0;
        for(const std::future<T> &rFuture: *this) { if(rFuture.wait_for(std::chrono::duration<double>::zero()) == std::future_status::ready) { ++num; } }
        return num;
    }
    ...
    [[nodiscard]] bool Valid() const {
        bool isValid = true;
        for(const std::future<T> &rFuture: *this) { isValid = isValid and rFuture.valid(); }
        return isValid;
    }
     ...
     */
    void Wait() const { for(const std::future<T> &rFuture: *this) { rFuture.wait(); } }
    ...
    bool WaitFor(const std::chrono::duration<R, P> &rDuration) const {
        ...
        for(const std::future<T> &rFuture: *this) {
            ...
            if(rDuration < std::chrono::steady_clock::now() - startTime) { return false; }
        }
        return true;
    }
    ...
    bool WaitUntil(const std::chrono::time_point<C, D> &rTimeoutTime) const {
        for(const std::future<T> &rFuture: *this) {
            ...
            if(rTimeoutTime < std::chrono::steady_clock::now()) { return false; }
        }
        return true;
    }
}; // class MultiFuture
...
public:
    ...
    void DetachBlock(const T firstIndex, const T indexAfterLast, Func &&rBlockFunc, const std::size_t blockNum = 0 THREADPOOL_PRIORITY_INPUT) {
        if(indexAfterLast > firstIndex) {
            ...
            for(std::size_t blk = 0; blk < blks.GetBlockNum(); ++blk) DetachTask([blockFunc = std::forward<Func>(rBlockFunc), start = blks.start(blk), end = blks.end(blk)] { blockFunc(start, end); } THREADPOOL_PRIORITY_OUTPUT);
        }
    }
    ...
    void DetachLoop(const T firstIndex, const T indexAfterLast, Func &&rLoopFunc, const std::size_t blockNum = 0 THREADPOOL_PRIORITY_INPUT) {
        if(indexAfterLast > firstIndex) {
            const Block blks(firstIndex, indexAfterLast, blockNum ? blockNum : m_threadNum);
            for(std::size_t blk = 0; blk < blks.GetBlockNum(); ++blk) { DetachTask([loopFunc = std::forward<Func>(rLoopFunc), start = blks.start(blk), end = blks.end(blk)] { for(T i = start; i < end; ++i) { loopFunc(i); } } THREADPOOL_PRIORITY_OUTPUT); }
        }
    }
     ...
     */
    template<typename T, typename Func>
    void DetachSequence(const T firstIndex, const T indexAfterLast, Func &&rSequenceFunc THREADPOOL_PRIORITY_INPUT) { for(T i = firstIndex; i < indexAfterLast; ++i) DetachTask([sequenceFunc = std::forward<Func>(rSequenceFunc), i] { sequenceFunc(i); } THREADPOOL_PRIORITY_OUTPUT); }
    ...
    [[nodiscard]] std::future<Result> SubmitTask(Func &&rTaskFunc THREADPOOL_PRIORITY_INPUT) {
        ...
        DetachTask([taskFunc = std::forward<Func>(rTaskFunc), pTaskPromise]() {
            ...
            try {
                ...
                } else { pTaskPromise->set_value(taskFunc()); }
#ifndef THREADPOOL_DISABLE_EXCEPTION_HANDLING
            } catch(...) { try { pTaskPromise->set_exception(std::current_exception()); } catch(...) {} }
#endif
 ...
 */
    ...
    [[nodiscard]] MultiFuture<Result> SubmitBlock(const T firstIndex, const T indexAfterLast, Func &&rBlockFunc, const std::size_t blockNum = 0 THREADPOOL_PRIORITY_INPUT) {
        if(indexAfterLast > firstIndex) {
            ...
            for(size_t blk = 0; blk < blks.GetBlockNum(); ++blk) future.push_back(SubmitTask([blockFunc = std::forward<Func>(rBlockFunc), start = blks.start(blk), end = blks.end(blk)] { return blockFunc(start, end); } THREADPOOL_PRIORITY_OUTPUT));
            return future;
        }
        return {};
    }
 ...
 */
    ...
    [[nodiscard]] MultiFuture<void> SubmitLoop(const T firstIndex, const T indexAfterLast, Func &&rLoopFunc, const std::size_t blockNum = 0 THREADPOOL_PRIORITY_INPUT) {
        if(indexAfterLast > firstIndex) {
            ...
            future.reserve(blks.GetBlockNum());
            for(std::size_t blk = 0; blk < blks.GetBlockNum(); ++blk) future.push_back(SubmitTask([loopFunc = std::forward<Func>(rLoopFunc), start = blks.start(blk), end = blks.end(blk)] { for(T i = start; i < end; ++i) { loopFunc(i); } } THREADPOOL_PRIORITY_OUTPUT));
            return future;
        }
        return {};
    }
 ...
 */
    ...
    [[nodiscard]] MultiFuture<Result> SubmitSequence(const T firstIndex, const T indexAfterLast, Func &&rSequenceFunc THREADPOOL_PRIORITY_INPUT) {
        if(indexAfterLast > firstIndex) {
            ...
            for(T i = firstIndex; i < indexAfterLast; ++i) future.push_back(SubmitTask([sequenceFunc = std::forward<Func>(rSequenceFunc), i] { return sequenceFunc(i); } THREADPOOL_PRIORITY_OUTPUT));
            return future;
        }
        return {};
    }
 ...
 */
    ...
    bool WaitFor(const std::chrono::duration<R, P> &rDuration) {
        ...
        m_isWaiting = true;
        const bool status = m_taskDoneCV.wait_for(lock, rDuration, [this] { return (m_runningTaskNum == 0) and PausedOrEmpty(); });
        m_isWaiting = false;
        return status;
    }
    ...
    bool WaitUntil(const std::chrono::time_point<C, D> &rTimeoutTime) {
        ...
        std::unique_lock lock(m_taskMutex);
        m_isWaiting = true;
        const bool status = m_taskDoneCV.wait_until(lock, rTimeoutTime, [this] { return (m_runningTaskNum == 0) and PausedOrEmpty(); });
        m_isWaiting = false;
...
private:
    ...
    public:
        ...
        Block(const T firstIndex, const T indexAfterLast, const size_t blockNum): m_firstIndex(firstIndex), m_indexAfterLast(indexAfterLast), m_blockNum(blockNum) {
            if(m_indexAfterLast > m_firstIndex) {
                ...
                if(m_blockNum > totalSize) m_blockNum = totalSize;
    ...
    public:
         ...
         */
        [[nodiscard]] T start(const std::size_t block) const { return m_firstIndex + static_cast<T>(block * m_blockSize) + static_cast<T>(block < m_remainder ? block : m_remainder); }
         ...
         */
        [[nodiscard]] T end(const std::size_t block) const { return (block == m_blockNum - 1) ? m_indexAfterLast : start(block + 1); }
         ...
         */
        [[nodiscard]] std::size_t GetBlockNum() const { return m_blockNum; }
