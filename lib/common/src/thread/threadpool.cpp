...
void ThreadPool::Init() { Init(0, [] {}); }

void ThreadPool::Init(const ConcurrencyT threadNum) { Init(threadNum, [] {}); }
...
void ThreadPool::Init(const std::function<void()> &rInitTask) {
    ...
    for(ConcurrencyT i = 0; i < m_threadNum; ++i) { m_pThreads[i] = std::thread(&ThreadPool::run, this, i, rInitTask); }
}
...
void ThreadPool::JoinAll() { for(ConcurrencyT i = 0; i < m_threadNum; ++i) { if(m_pThreads[i].joinable()) { m_pThreads[i].join(); } } }
...
std::vector<std::thread::native_handle_type> ThreadPool::GetNativeHandle() const {
    ...
    for(ConcurrencyT i = 0; i < this->m_threadNum; ++i) { nativeHandleVec[i] = this->m_pThreads[i].native_handle(); }
...
ConcurrencyT ThreadPool::GetThreadNum() const { return m_threadNum; }

std::vector<std::thread::id> ThreadPool::GetThreadIds() const {
    std::vector<std::thread::id> threadIds(m_threadNum);
    for(ConcurrencyT i = 0; i < m_threadNum; ++i) { threadIds[i] = m_pThreads[i].get_id(); }
    return threadIds;
}
...
void ThreadPool::Reset() { Reset(0, [] {}); }

void ThreadPool::Reset(const ConcurrencyT threadsNum) { Reset(threadsNum, [] {}); }

void ThreadPool::Reset(const std::function<void()> &rInitTaskFunc) { Reset(0, rInitTaskFunc); }
...
void ThreadPool::Wait() {
    ...
    m_isWaiting = true;
    m_taskDoneCV.wait(lock, [this] { return (this->m_runningTaskNum == 0) and PausedOrEmpty(); });
    m_isWaiting = false;
}

ConcurrencyT ThreadPool::determineThreadNum(const ConcurrencyT threadsNum) {
    if(threadsNum > 0) { return threadsNum; }
...
void ThreadPool::run(const ConcurrencyT idx, const std::function<void()> &rInitTaskFunc) {
    ...
    while(true) {
        --m_runningTaskNum;
        lock.unlock();
        if(m_isWaiting and (m_runningTaskNum == 0) and PausedOrEmpty()) { m_taskDoneCV.notify_all(); }
        lock.lock();

        m_taskAvailableCV.wait(lock, [this] { return !PausedOrEmpty() or !m_isRunning; });
...
OptionalPool ThreadInfoPool::operator()() const { return m_pool; }
} // namespace this_thread

class GlobalThreadPool: public shadow::singleton::Singleton<GlobalThreadPool> {
public:
    GlobalThreadPool() { m_pThreadPool = std::make_unique<ThreadPool>(); }

    ThreadPool *GetThreadPool() const { return m_pThreadPool.get(); }

private:
    std::unique_ptr<ThreadPool> m_pThreadPool{nullptr};
}; // class ThreadPoolManager

ThreadPool *GetGlobalThreadPool() { return GlobalThreadPool::GetInstance()->GetThreadPool(); }
